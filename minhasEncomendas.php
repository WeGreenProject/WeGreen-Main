...
<body>
        ...
        <main class="main-content">
            ...
            <div class="page-content">
                ...
                <div class="content-area">
                    ...
                    <div class="filters-container" style="background: #fff; padding: 20px; border-radius: 12px; margin-bottom: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                        <div style="display: grid; grid-template-columns: 2fr 1fr 1fr 1fr auto; gap: 15px; align-items: end;">
                            <div>
                                ...
                                <input type="text" id="searchProduct" placeholder="Nome do produto..." style="width: 100%; padding: 10px; border: 1px solid #e0e0e0; border-radius: 8px;">
                            </div>
                            <div>
                                <label style="display: block; margin-bottom: 8px; font-size: 14px; color: #666;">Status</label>
                                <select id="filterStatus" class="filter-select" style="width: 100%; padding: 10px; border: 1px solid #e0e0e0; border-radius: 8px;">
                            ...
                            <div>
                                ...
                                <select id="filterPeriod" style="width: 100%; padding: 10px; border: 1px solid #e0e0e0; border-radius: 8px;">
                            ...
                            <button class="btn-secondary" onclick="limparFiltros()" style="padding: 10px 20px; background: #f5f5f5; border: none; border-radius: 8px; cursor: pointer;">
                    ...
                    <!-- Mensagem vazia -->
                    <div id="emptyState" style="display: none; text-align: center; padding: 60px; background: #fff; border-radius: 12px;">
                        ...
                        <button class="btn-primary" onclick="window.location.href='index.html'" style="margin-top: 20px; padding: 12px 24px; background: #3cb371; color: white; border: none; border-radius: 8px; cursor: pointer;">
    ...
    <script>
        ...
        function carregarEncomendas() {
            $.ajax({
                ...
                method: 'POST',
                data: { op: 'listarEncomendasCliente' },
                dataType: 'json',
        ...
        function criarTimeline(estado) {
            const steps = [
                ...
                { key: 'processando', icon: 'cog', label: 'Processando' },
                { key: 'enviado', icon: 'truck', label: 'Enviado' },
                { key: 'entregue', icon: 'check-circle', label: 'Entregue' }
            ];
        ...
        function aplicarFiltros() {
            ...
            filtradas.sort((a, b) => {
                switch(sort) {
                    case 'date-desc': return new Date(b.data_envio) - new Date(a.data_envio);
                    case 'date-asc': return new Date(a.data_envio) - new Date(b.data_envio);
                    case 'value-desc': return parseFloat(b.total) - parseFloat(a.total);
                    case 'value-asc': return parseFloat(a.total) - parseFloat(b.total);
                    default: return 0;
                }
            });
        ...
        function verDetalhes(codigo) {
            $.ajax({
                url: 'src/controller/controllerEncomendas.php',
                method: 'POST',
                data: { op: 'detalhesEncomenda', codigo: codigo },
                dataType: 'json',
                success: function(response) {
                    if(response.success) {
        ...
        function gerarPDFFatura(encomenda) {
            ...
            doc.text('€' + parseFloat(encomenda.total).toFixed(2), 180, finalY, { align: 'right' });
            ...
            doc.text('Obrigado pela sua compra! WeGreen - Sustentabilidade em cada produto.', 105, 280, { align: 'center' });
            doc.text('www.wegreen.pt | suporte@wegreen.pt', 105, 285, { align: 'center' });
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'src/controller/controllerEncomendas.php',
                        method: 'POST',
                        data: { op: 'cancelarEncomenda', codigo: codigo },
                        dataType: 'json',
                        success: function(response) {
                            if(response.success) {
    ...
    <style>
        ...
        @keyframes pulse {
            0%, 100% { box-shadow: 0 0 0 4px rgba(60,179,113,0.2); }
            50% { box-shadow: 0 0 0 8px rgba(60,179,113,0.1); }
        }
