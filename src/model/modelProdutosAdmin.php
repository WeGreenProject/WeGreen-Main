...
class ProdutosAdmin{
    ...
    function getProdutosPendentes(){
        ...
        $msg = "";
        $row = "";
        $sql = "SELECT Count(*) As ProdutosCount from Produtos  where produtos.estado like 'Aprovado';";
        ...
        $result = $conn->query($sql);
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    $msg .= "<div class='approval-stat-info'>";
                    $msg  .= "<div class='approval-stat-value'>".$row["ProdutosCount"]."</div>";
                    $msg .= "<div class='approval-stat-label'>Aprovados</div>";
                    $msg .= "</div>";
                }
            }
            else
        ...
        $result = $conn->query($sql);
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    $msg .= "<div class='approval-stat-info'>";
                    $msg  .= "<div class='approval-stat-value'>".$row["ProdutosCount"]."</div>";
                    $msg .= "<div class='approval-stat-label'>Rejeitados</div>";
                    $msg .= "</div>";
                }
            }
            else
