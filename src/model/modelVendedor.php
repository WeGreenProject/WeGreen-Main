...
class Vendedor {
    ...
    function getPerfilVendedora($utilizador_id) {
        ...
        $msg = "";
        $row = "";
        $sql = "SELECT ranking.nome As rankname,utilizadores.* FROM Utilizadores,ranking WHERE Utilizadores.ranking_id = ranking.id AND Utilizadores.id = $utilizador_id";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $msg .= "<img src='".$row["foto"]."' class='rounded-circle border border-2 border-success shadow-sm' width='110' height='110' style='object-fit: cover;'>";
                $msg .= "<div>";
                $msg .= "<h4 class='fw-bold text-success mb-1'>".$row["nome"]."</h4>";
                $msg .= "<p class='text-muted mb-1 d-flex align-items-center'>";
                $msg .= "<i class='bi bi-geo-alt-fill text-success me-1'></i> ".$row["password"];
                $msg .= "</p>";
                $msg .= "<p class='mb-1'>";
                $msg .= "<span class='badge bg-success-subtle text-success border border-success fw-semibold px-3'>".$row["rankname"]."</span>";
                $msg .= "</p>";
                $msg .= "<p class='text-muted small mb-1'>";
                $msg .= "<strong>".$row["pontos_conf"]."</strong> Pontos · ";
