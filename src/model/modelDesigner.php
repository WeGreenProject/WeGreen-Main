...
class Designer {
    ...
    function getFiltrosDesignerTamanho(){
        ...
        $sql = "SELECT DISTINCT tamanho AS NomeTamanho,
                tamanho AS ValueTamanho FROM Produtos WHERE designer_id IS NOT NULL AND Produtos.ativo = 1 ORDER BY tamanho;";
    ...
    function getFiltrosDesignerEstado(){
        ...
        $sql = "SELECT DISTINCT estado AS NomeEstado,
                estado AS ValueEstado FROM Produtos WHERE designer_id IS NOT NULL AND Produtos.ativo = 1 ORDER BY estado;";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
        $result = $stmt->get_result();
...
function getProdutosDesigner($categoria,$tamanho,$estado){
    ...
    $sql = "SELECT * FROM Produtos WHERE ativo = 1 AND designer_id IS NOT NULL";
